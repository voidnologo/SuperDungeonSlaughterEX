defmodule SuperDungeonSlaughterExWeb.GameLive do
  ...
  def render(assigns) do
    ~H"""
    <div class="min-h-screen bg-base-100 text-primary font-mono">
      <%= if @show_name_prompt do %>
        <div class="flex items-center justify-center min-h-screen">
          <div class="bg-base-200 border-4 border-warning rounded-lg p-8 max-w-md w-full mx-4">
            <h2 class="text-3xl font-bold text-error text-center mb-6">
              Super Dungeon Slaughter EX
            </h2>
            <.form for={@form} id="hero-name-form" phx-change="validate" phx-submit="create_hero">
              <div class="space-y-4">
                <div>
                  <label class="block text-primary mb-2">What is your hero's name?</label>
                  <input
                    type="text"
                    name="hero[name]"
                    id="hero_name"
                    ...
                    autofocus
                    class="w-full px-4 py-2 bg-base-300 border-2 border-primary text-primary rounded focus:outline-none focus:border-primary"
                  />
                </div>
                <div>
                  <label class="block text-primary mb-2">Select Difficulty:</label>
                  <div class="grid grid-cols-3 gap-2">
                    <label class={[
                      ...
                      Phoenix.HTML.Form.input_value(@form, :difficulty) != "easy" &&
                        "border-base-300 bg-base-200 hover:border-info"
                    ]}>
                      <input
                        type="radio"
                        ...
                        class="sr-only"
                      />
                      <div class="text-info font-bold">Easy</div>
                      <div class="text-xs text-base-content/60">-5-10%</div>
                    </label>
                    <label class={[
                      "relative cursor-pointer rounded-lg border-2 p-3 text-center transition-all",
                      ...
                      Phoenix.HTML.Form.input_value(@form, :difficulty) != "normal" &&
                        "border-base-300 bg-base-200 hover:border-success"
                    ]}>
                      <input
                        type="radio"
                        name="hero[difficulty]"
                        ...
                        class="sr-only"
                      />
                      <div class="text-success font-bold">Normal</div>
                      <div class="text-xs text-base-content/60">Standard</div>
                    </label>
                    <label class={[
                      "relative cursor-pointer rounded-lg border-2 p-3 text-center transition-all",
                      ...
                      Phoenix.HTML.Form.input_value(@form, :difficulty) != "hard" &&
                        "border-base-300 bg-base-200 hover:border-error"
                    ]}>
                      <input
                        type="radio"
                        name="hero[difficulty]"
                        ...
                        class="sr-only"
                      />
                      <div class="text-error font-bold">Hard</div>
                      <div class="text-xs text-base-content/60">+5-10%</div>
                    </label>
                  </div>
                </div>
                <button
                  type="submit"
                  class="w-full py-3 bg-success hover:bg-success/80 text-success-content text-xl font-bold rounded transition-colors"
                >
                  Begin Adventure
                </button>
                <button
                  ...
                  class="w-full py-3 bg-secondary hover:bg-secondary/80 text-secondary-content text-xl font-bold rounded transition-colors"
                >
                  View High Scores
                </button>
              </div>
            ...
            <button
              ...
              class="absolute bottom-4 right-4 text-warning hover:text-warning/80 font-bold transition-colors"
            >
              Settings
            </button>
          </div>
      ...
      <% else %>
        ...
        <header class="text-center py-6">
          <h1 class="text-4xl font-bold text-error drop-shadow-lg">
            Super Dungeon Slaughter EX
          </h1>
        </header>
    ...
    <!-- Action Buttons -->
          <div class="flex gap-4 justify-center mt-8">
            <button
              ...
              class="px-8 py-4 bg-success hover:bg-success/80 disabled:opacity-50 disabled:cursor-not-allowed text-success-content text-2xl font-bold rounded-lg transition-colors shadow-lg"
            >
              REST
            </button>
            <button
              phx-click="fight"
              disabled={@game_state.game_over}
              class="px-8 py-4 bg-error hover:bg-error/80 disabled:opacity-50 disabled:cursor-not-allowed text-error-content text-2xl font-bold rounded-lg transition-colors shadow-lg"
            >
              FIGHT
            </button>
          </div>
    ...
    <!-- Game Over Modal Overlay -->
      ...
      <!-- Settings Modal (available from start screen) -->
      <%= if @show_settings do %>
        <.settings_modal />
      <% end %>

      <!-- High Scores Modal (available from start screen) -->
      <%= if @show_name_prompt and @show_high_scores do %>
